'use strict';
const wechat_file = __dirname + '/wechat.txt';
const wechat_ticket_file = __dirname + '/wechat_ticket.txt';
...
module.exports = {
    ...
    appSecret: 'efaf8f6beb08b10f94bcb575c9b29478',//61b26f9a5d9928ca79520ec6f408c533
    token: 'chensonghao',
    ...
    ticketUrl(access_token,type){
        return `${prefix}ticket/getticket?access_token=${access_token}&type=${type}`;
    },
    getAccessToken() {
    ...
    menu: {
        ...
        getAllMenuUrl(access_token) {
            return `${prefix}get_current_selfmenu_info?access_token=${access_token}`;
        },
        deleteMenuUrl(access_token) {
            return `${prefix}menu/delete?access_token=${access_token}`;
        }
    },
    conditionalMenu: {
        createMenuUrl(access_token) {
            return `${prefix}menu/addconditional?access_token=${access_token}`;
        },
        deleteMenuUrl(access_token) {
            return `${prefix}menu/delconditional?access_token=${access_token}`;
        }
    },
    qrCode: {
        shortUrl(access_token){
            return `${prefix}shorturl?access_token=${access_token}`;
        },
        createQrCodeUrl(access_token) {
            return `${prefix}qrcode/create?access_token=${access_token}`;
        },
        getQrCodeUrl(ticket) {
